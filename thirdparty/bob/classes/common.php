...
class Common
{
	...
	public static function load_template($template_name)
	{
		...
		if(File::exists($source = Config::get('thirdparty_bob::options.project_templates').$template_name))
		{
			return File::get($source);
		}
		elseif(File::exists($source = Config::get('thirdparty_bob::options.template_path').$template_name))
		{
			return File::get($source);
		}
		else
	...
	{
		static::$_ansi = Config::get('thirdparty_bob::options.ansi_support');
